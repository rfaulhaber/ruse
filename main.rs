...
use ruse::{Lexer, Parser, SourceFile, Span};
...
fn main() -> Result<()> {
    ...
    match lexer.tokenize() {
        ...
        Err(e) => {
            return Err(miette::Report::new(e).with_source_code(NamedSource::new("input", input)));
        }
    }
    ...
    if let Err(e) = lexer.tokenize() {
        let report =
            miette::Report::new(e).with_source_code(NamedSource::new("bad_input1.scm", bad_input1));
        eprintln!("{}\n", report);
    }
    ...
    if let Err(e) = lexer.tokenize() {
        let report =
            miette::Report::new(e).with_source_code(NamedSource::new("bad_input2.scm", bad_input2));
        eprintln!("{}\n", report);
    }
